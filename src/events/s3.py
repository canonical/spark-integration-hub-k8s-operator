...
class S3Events(BaseEventHandler, WithLogging):
    ...
    def _on_s3_credential_gone(self, _: CredentialsGoneEvent):
        ...
        self.integration_hub.update(
            None, self.context.azure_storage, self.context.pushgateway, self.context.hub_configurations
        )

        self.charm.unit.status = self.get_app_status(None, self.context.azure_storage, self.context.pushgateway)
        if self.charm.unit.is_leader():
            self.charm.app.status = self.get_app_status(None, self.context.azure_storage, self.context.pushgateway)