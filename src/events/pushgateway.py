...
class PushgatewayEvents(BaseEventHandler, WithLogging):
    ...
    def _on_pushgateway_gone(self, _: RelationBrokenEvent):
        ...
        self.integration_hub.update(self.context.s3, self.context.azure_storage, None, self.context.hub_configurations)

        self.charm.unit.status = self.get_app_status(self.context.s3, self.context.azure_storage, None)
        if self.charm.unit.is_leader():
            self.charm.app.status = self.get_app_status(self.context.s3, self.context.azure_storage, None)
